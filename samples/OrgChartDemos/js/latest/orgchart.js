...
OrgChart.prototype.load = function(t) {
    ...
    this.server.read(t, this.width(), this.height(), e, O, v, function(t) {
        O != OrgChart.action.exporting && (x.nodes = t.nodes), x.editUI.fields = t.allFields;
        var e = {
            defs: ""
        };
        ...
        for (var n in OrgChart.events.publish("prerender", [x, e]), r = e.content, t.visibleNodes) {
            var l = t.visibleNodes[n],
                s = x._get(n);
            r += x.ui.node(l, s, t.animations, x.config, void 0, void 0, void 0, O, i, x), r += x.ui.link(l, x, x.config, i), r += x.ui.expandCollapse(l, x.config, O, i)
        }
        e = {
            content: r,
            res: t
        };
        if (OrgChart.events.publish("render", [x, e]), r = e.content, r += x.ui.lonely(x.config), O !== OrgChart.action.exporting) {
            O !== OrgChart.action.centerNode && O !== OrgChart.action.insert && O !== OrgChart.action.expandCollapse && O !== OrgChart.action.update || (o = a), O === OrgChart.action.init && null != a && (o = a), x.response = t;
            m = x.ui.svg(x.width(), x.height(), o, x.config, r);
            if (x._initialized) {
                var h = x.getSvg(),
                    d = h.parentNode;
                d.removeChild(h), d.insertAdjacentHTML("afterbegin", m), x._attachEventHandlers(), x.xScrollUI.addListener(x.getSvg()), x.yScrollUI.addListener(x.getSvg()), x.xScrollUI.setPosition(), x.yScrollUI.setPosition()
            } else x.element.innerHTML = x.ui.css() + x.ui.exportMenuButton(x.config) + m, x._attachInitEventHandlers(), x._attachEventHandlers(), x.xScrollUI.create(x.width(), x.config.padding), x.xScrollUI.setPosition(), x.xScrollUI.addListener(x.getSvg()), x.yScrollUI.create(x.height(), x.config.padding), x.yScrollUI.setPosition(), x.yScrollUI.addListener(x.getSvg()), x.config.enableSearch && x.searchUI.addSearchControl(), x.toolbarUI.init(x, x.config.toolbar);
            var c = !1;
            x.notifierUI.show(t.notif);
            var g = x.response.animations;
            if (0 < g[0].length) {
                x._hideBeforeAnimation(g[0].length);
                for (var p = 0; p < g[0].length; p++) g[0][p] = x.getNodeElement(g[0][p]);
                OrgChart.anim(g[0], g[1], g[2], x.config.anim.duration, x.config.anim.func, function() {
                    c || (y && y(), OrgChart.events.publish("redraw", [x]), x._showAfterAnimation(), c = !0)
                })
            }
            O === OrgChart.action.centerNode ? OrgChart.anim(x.getSvg(), {
                viewbox: a
            }, {
                viewbox: x.response.viewBox
            }, x.config.anim.duration, x.config.anim.func, function() {
                x.ripple(v.options.rippleId), c || (y && y(), OrgChart.events.publish("redraw", [x]), x._showAfterAnimation(), c = !0)
            }, function() {
                x.xScrollUI.setPosition(), x.yScrollUI.setPosition()
            }) : !a || !x.response || a[0] == x.response.viewBox[0] && a[1] == x.response.viewBox[1] && a[2] == x.response.viewBox[2] && a[3] == x.response.viewBox[3] || O !== OrgChart.action.insert && O !== OrgChart.action.expandCollapse && O !== OrgChart.action.update && O !== OrgChart.action.init ? 0 == g[0].length && (c || (y && y(), OrgChart.events.publish("redraw", [x]), c = !0)) : OrgChart.anim(x.getSvg(), {
                viewbox: a
            }, {
                viewbox: x.response.viewBox
            }, 500, OrgChart.anim.inOutPow, function() {
                x.xScrollUI.setPosition(), x.yScrollUI.setPosition(), c || (y && y(), OrgChart.events.publish("redraw", [x]), c = !0)
    ...
    if (!0 === i && !1 === OrgChart.events.publish("group", [this, t, e])) return !1;
    ...
    if (a.isGroup || o.isGroup || a.isChildOfGroup || o.isChildOfGroup)
        if (a.isGroup || o.isGroup || !a.isChildOfGroup || o.isChildOfGroup)
            if (a.isGroup || o.isGroup || a.isChildOfGroup || !o.isChildOfGroup)
                if (!a.isGroup && !o.isGroup && a.isChildOfGroup && o.isChildOfGroup) this._removeGroupIfHasOneNode(a._groupParentNodeId, a.id), this.removeNodeTag(a.id, a._groupParentNodeId, !0), this.addNodeTag(a.id, o._groupParentNodeId, !0), this.link(a.id, o.pid);
                else if (!a.isGroup && o.isGroup && a.isChildOfGroup && !o.isChildOfGroup) this._removeGroupIfHasOneNode(a._groupParentNodeId, a.id), this.removeNodeTag(a.id, a._groupParentNodeId, !0), this.addNodeTag(a.id, o.id, !0);
    else if (a.isGroup || !o.isGroup || a.isChildOfGroup || o.isChildOfGroup) {
        if (a.isGroup && o.isGroup && !a.isChildOfGroup && !o.isChildOfGroup)
            for (var l = 0; l < a.bgnodes.length; l++) this.removeNodeTag(a.bgnodes[l].id, a.id, !0), this.addNodeTag(a.bgnodes[l].id, o.id, !0)
    } else this.addNodeTag(a.id, o.id, !0);
    else this.addNodeTag(a.id, o._groupParentNodeId, !0), this.link(a.id, o.pid);
    else {
        ...
        this.addTag(s, {
            ...
            template: "group_grey"
        }, !0), this._removeGroupIfHasOneNode(a._groupParentNodeId, a.id), this.removeNodeTag(a.id, a._groupParentNodeId, !0), this.addNodeTag(a.id, s, !0), this.addNodeTag(o.id, s, !0)
    } else {
        ...
        this.addTag(s, {
            ...
            groupState: OrgChart.EXPAND,
            template: "group_grey"
        }, !0), this.addNodeTag(a.id, s, !0), this.addNodeTag(o.id, s, !0);
    ...
    if (d.innerHTML = "Upload", d.style.position = "absolute", d.style.right = 0, n.style.margin = "14px 14px 7px 14px", n.style.textAlign = "left", n.style.position = "relative", n.setAttribute("data-field-name", t), h.style.border = "1px solid #d7d7d7", h.style.backgroundColor = "#d7d7d7", h.style.display = "block", h.style.width = "100%", l.style.color = "#bcbcbc", s.style.border = "none", s.style.outline = "none", s.style.width = "100%", s.setAttribute("val", ""), a && (s.style.width = "80%"), s.style.fontSize = "16px", s.readOnly = i, null != e && null != e && (s.value = e), null != r && null != r && (s.placeholder = r), null != t && null != t && (l.innerHTML = t), l.setAttribute("lbl", ""), s.style.color = "#7a7a7a", i || s.addEventListener("focus", function() {
            ...
            t.setAttribute("type", "file"), t.style.display = "none", t.onchange = function() {
                var t = this.files[0];
                OrgChart.events.publish("imageuploaded", [o, t, e.parentNode.querySelector("input")])
            }, document.body.appendChild(t), t.click()
        }), s.addEventListener("blur", function() {
            this.parentNode.getElementsByTagName("hr")[0].style.border = "1px solid #d7d7d7"
        }), n.appendChild(l), n.appendChild(s), a && n.appendChild(d), n.appendChild(h), OrgChart.addValidation) {
        var c = {
            wrapper: n,
            label: l,
            input: s,
    ...
    return "bottom" == a ? o = t.rightNeighbor ? t.y + t.h + (t.rightNeighbor.y - (t.y + t.h)) / 2 : t.y + t.h + r.siblingSeparation / 2 : "top" == a && (o = t.leftNeighbor ? t.y + (t.leftNeighbor.y + t.leftNeighbor.h - t.y) / 2 : t.y - r.siblingSeparation / 2), e.push([e[e.length - 1][0], o]), e.push([t.x - r.levelSeparation / 3, o]), n = e[e.length - 1][0], s = o, i.p = n, i.q = o, i.s = s, i
}, OrgChart._addPointRight = function(t, e, r, i, a) {
    var o, n = e[e.length - 1][0];
    return "bottom" == a ? o = t.rightNeighbor ? t.y + t.h + (t.rightNeighbor.y - (t.y + t.h)) / 2 : t.y + t.h + r.siblingSeparation / 2 : "top" == a && (o = t.leftNeighbor ? t.y + (t.leftNeighbor.y + t.leftNeighbor.h - t.y) / 2 : t.y - r.siblingSeparation / 2), e.push([e[e.length - 1][0], o]), e.push([t.x + t.w + r.levelSeparation / 3, o]), n = e[e.length - 1][0], s = o, i.p = n, i.q = o, i.s = s, i
    ...
    for (var h = 0; h < l.length; h++) {
        ...
        } else this._addField(o, OrgChart.input(l[h], d, null, !0, void 0, r), l[h], "details")
    }
    ...
    for (var c = 0; c < h.length; c++) {
        ...
        if (t.isGroup && (g = this.obj.config.tags[t.id]["group" + h[c]]), "tags" != h[c])
            if (OrgChart._fieldIsImg(this.obj.config, h[c])) {
                ...
                this._addField(n, OrgChart.input(h[c], g, null, !1, !0, i), h[c], "edit")
            } else this._addField(n, OrgChart.input(h[c], g, null, !1, void 0, i), h[c], "edit")
    }
    ...
    this._draw(!1, OrgChart.action.exporting, t, function(t) {
        ...
        if (r.innerHTML = t, "svg" == a.ext)
            if (o) o(a, r.innerHTML);
                ...
                if (!1 === i) return !1;
                if (OrgChart._downloadFile(a.mime, e.content, e.options.filename, e.options.openInNewTab), !1 === (i = OrgChart.events.publish("exportend", [n, e]))) return !1
            }
        else n._pages(a, r.querySelector("svg"), function(t) {
            var e = {
                content: r.innerHTML,
                options: a,
                pages: t
            ...
            o || OrgChart.loading.show(n), o ? o(n, e, r.querySelector("svg")) : (e = JSON.stringify(e), OrgChart._ajax(n.config.exportUrl + "/v3", "POST", e, "arraybuffer", function(t) {
                ...
                OrgChart.loading.hide(n), OrgChart._downloadFile(a.mime, t, a.filename, a.openInNewTab)
            }))
        })
    })
    ...
    for (var e in this.nodes) {
        this.destroy, (0 != (i = this.nodes[e]).childrenIds.length || 0 == i.childrenIds.length && null == i.pid) && (i.isAssistant || "split" == i.templateName || g.push(i.id))
    }
        ...
        this.config.roots = [i.id],
            function(r) {
                s._draw(!1, OrgChart.action.exporting, t, function(t) {
                    ...
                    e.innerHTML = t,
                        function(t, e) {
                            for (var r = t.querySelector("svg"), i = s._pagesAfit(n, r), a = 0; a < i.length; a++) {
                                var o = i[a];
                                o.html = t.innerHTML, c.push(o)
                            }
                            g.length - 1 == e && (s.config.collapse = h, s.config.roots = d, l(c))
                        }(e, r)
                })
            }(r)
    }
    ...
    for (var e in this.nodes) {
        var r = this.nodes[e];
        (0 != r.childrenIds.length || 0 == r.childrenIds.length && null == r.pid) && (r.isAssistant || "split" == r.templateName || g.push(r.id))
    }
        ...
        this.config.roots = [e],
            function(r) {
                s._draw(!1, OrgChart.action.exporting, t, function(t) {
                    ...
                    e.innerHTML = t,
                        function(t, e) {
                            for (var r = t.querySelector("svg"), i = s._pagesFit(n, r), a = 0; a < i.length; a++) {
                                var o = i[a];
                                o.html = t.innerHTML, c.push(o)
                            }
                            g.length - 1 == e && (s.config.collapse = h, s.config.roots = d, l(c))
    ...
    for (var g = n, p = l, f = d.w * (100 / r), u = d.h * (100 / r), C = []; g < s;) {
        for (; p < h;) {
            var m = [g, p, g + f, u];
            m = m.join(), C.push({
                vb: m,
    ...
    if (this.config.enableDragDrop) {
        ...
        if (!1 !== OrgChart.events.publish("drag", [this, i])) {
            ...
            var O = function(t, e) {
                ...
                y = function(t) {
                    if (c.removeEventListener(n.move, v), c.removeEventListener(n.up, y), n.leave && c.removeEventListener(n.leave, y), l.id == s || l.isChildOfGroup && l._groupParentNodeId == s || null == s) return O(s, h), c.removeChild(m), void(d._gragStartedId = null);
                    var e = d.getBGNode(s);
                    ...
                    if (null == d.config.dragDropMenu || l.isGroup && !e.isGroup) d._canUpdateLink(l.id, s) ? d.linkNode(l.id, s) : (c.removeChild(m), O(s, h));
            ...
            c.addEventListener(n.move, v), c.addEventListener(n.up, y), n.leave && c.addEventListener(n.leave, y)
        }
    }
    ...
    if (!1 === OrgChart.events.publish("click", [this, this.getBGNode(t)])) return !1;
    ...
    return l
}, void 0 === OrgChart && (OrgChart = {}), OrgChart.server._createNodes = function(a, t, e, r) {
    for (var i = a.nodes, o = {}, n = [], l = [], s = [], h = {}, d = !1, c = 0; c < i.length; c++) {
        var g = i[c],
            p = Array.isArray(g.tags) ? g.tags.slice(0) : [],
            f = OrgChart._getGroup(p, a.tags),
            u = OrgChart._getTemplate(p, a.tags, a.template);
        if ((v = new OrgChart.node(g.id, g.pid, p, u)).isAssistant = -1 != p.indexOf("assistant"), v.isAssistant && -1 == s.indexOf(v.pid) && OrgChart.server._isNotNullEmptyUndefined(v.pid) && s.push(v.pid), null != f) {
            if (!o[f.name]) {
                n.push(f.name);
                var C = new OrgChart.node(f.name, g.pid, [f.name], f.tag.template);
                C.isGroup = !0, null != a.orderBy && (C.order = OrgChart.server._getOrderFieldValue(g, a.orderBy)), o[f.name] = C
            }
        ...
        } else null != a.orderBy && (v.order = OrgChart.server._getOrderFieldValue(g, a.orderBy)), o[g.id] = v, n.push(g.id)
    }
    if (null != a.orderBy && n.sort(function(t, e) {
            var r = o[t].order,
                i = o[e].order;
    ...
    for (c = 0; c < n.length; c++) {
        var O = n[c],
            v = o[O],
            y = r ? r[O] : null;
        if (OrgChart.server._isNotNullEmptyUndefined(v.pid))(H = o[v.pid]) ? ((v.parent = H).children.push(v), H.childrenIds.push(v.id)) : (v.pid = null, v.parent = null, v.isAssistant = !1, l.push(v));
        else v.isAssistant = !1, l.push(v);
        t == OrgChart.action.exporting && null != e.id && (l = [o[e.id]]), y ? (v.state = y.state, v.state == OrgChart.COLLAPSE && m.push(v.id), OrgChart.server._setNodeGroupState(v, y.groupState, a.orientation)) : OrgChart.server._setNodeGroupState(v, OrgChart.EXPAND, a.orientation), OrgChart.server._initStates(v, o, t, a, e, m), OrgChart.server._initGroupStates(v, o, t, a, e)
    }
    if (null != a.roots) {
        l = [];
            ...
            if (x) {
                if (x.parent)
                    for (var b = (H = x.parent).children.length - 1; 0 <= b; b--) {
                        (z = H.children[b]) == x && (z.parent = null, H.children.splice(b, 1), H.childrenIds.splice(b, 1))
                    }
    ...
    for (c = 0; c < s.length; c++) {
        ...
        if (H) {
            ...
            (o[_.id] = _).isAssistant = !0, (y = r[_.id]) && (o[_.id].state = y.state, o[_.id].state == OrgChart.COLLAPSE && m.push(_.id)), OrgChart.server._initStates(_, o, t, a, e, m);
            var k = [];
            for (b = H.children.length - 1; 0 <= b; b--) {
                (z = H.children[b]).isAssistant ? (z.parent = null, H.children.splice(b, 1), H.childrenIds.splice(b, 1), k.splice(0, 0, z.id)) : (z.pid = _.id, (z.parent = _).children.unshift(z), _.childrenIds.unshift(z.id), H.children.splice(b, 1))
            }
            if (k.length % 2) {
                ...
                OrgChart.templates[I] = {
                    ...
                    nodeMenuButton: "",
                    size: S
                }, (W = new OrgChart.node(H.id + "_transperant", A.pid, [], I)).isAssistant = !0, o[W.id] = W, k.splice(k.length - 1, 0, W.id)
            }
            var N = 1;
            for (b = k.length - 1; 0 <= b; b--) {
                if (b % 2 && b != k.length - 1)(J = new OrgChart.node(H.id + "_split_" + N, null, [], "split")).isAssistant = !0, o[J.id] = J, k.splice(b, 0, J.id), N++;
    ...
    if (t == OrgChart.action.centerNode) {
        var P = o[e.id];
        P || (P = o[h[e.id]], OrgChart.server._setNodeGroupState(P, OrgChart.EXPAND, a.orientation)), P.state = OrgChart.EXPAND, -1 != (D = m.indexOf(P.id)) && m.splice(D, 1);
        for (var H = OrgChart.server._getRealParent(P), U = P.id; null != H;) {
            if (-1 != (D = m.indexOf(H.id)) && m.splice(D, 1), H.state = OrgChart.EXPAND, e.options.parentState === OrgChart.COLLAPSE_PARENT_NEIGHBORS)
                for (c = 0; c < H.childrenIds.length; c++) {
                    var z = o[H.childrenIds[c]];
                    if (!OrgChart.server._isSplit(z) && !z.isAssistant && z.id != U) - 1 == (D = m.indexOf(H.childrenIds[c])) && m.push(H.childrenIds[c]), z.state = OrgChart.COLLAPSE
                }
            if (H.state == OrgChart.EXPAND)
                for (var F = 0; F < H.childrenIds.length; F++) {
                    var j = o[H.childrenIds[F]];
                    if (OrgChart.server._isSplit(j) || j.isAssistant) - 1 != (D = m.indexOf(j.id)) && m.splice(D, 1), j.state = OrgChart.EXPAND
                }
        ...
        if (e.options.childrenState === OrgChart.COLLAPSE_SUB_CHILDRENS)
            for (c = 0; c < P.childrenIds.length; c++) {
                if (-1 != (D = m.indexOf(P.childrenIds[c])) && m.splice(D, 1), (z = o[P.childrenIds[c]]).state = OrgChart.EXPAND, !z.isAssistant && !OrgChart.server._isSplit(z))
                    for (b = 0; b < z.childrenIds.length; b++) {
                        var D, G = o[z.childrenIds[b]]; - 1 == (D = m.indexOf(z.childrenIds[b])) && m.push(z.childrenIds[b]), G.state = OrgChart.COLLAPSE
                    }
            }
    }
    ...
    for (c = 0; c < m.length; c++) {
        if (H = (v = o[m[c]]).parent)
            for (b = H.children.length - 1; 0 <= b; b--) {
                if (H.children[b] == v)
                    if (-1 == H.collapsedChildrenIds.indexOf(v.id) && H.collapsedChildrenIds.push(v.id), H.children.splice(b, 1), "split" == v.templateName || "split" == H.templateName) {
                        if (M = v.parent) {
                            for (;
                                "split" == M.templateName;) M = M.parent;
                            for (var X = M.childrenIds.length - 1; 0 <= X; X--) {
                                var R = M.childrenIds[X]; - 1 == M.collapsedChildrenIds.indexOf(R) && M.collapsedChildrenIds.push(R)
                            }
                        }
                    } else v.parent = null
            }
        ...
        for (c = 0; c < n.length; c++) {
            if (0 == (v = o[n[c]]).childrenIds.length && !v.isAssistant && "split" != v.templateName)
                if (H = v.parent) {
                    ...
                    for (b = H.children.length - 1; 0 <= b; b--) {
                        if ((z = H.children[b]) != v && 0 == z.childrenIds.length && !z.isAssistant && "split" != z.templateName) {
                            z.parent = v, z.pid = v.id, z.layout = OrgChart.mixed, H.children.splice(b, 1);
    ...
    if (a.layout == OrgChart.tree || a.layout == OrgChart.treeRightOffset || a.layout == OrgChart.treeLeftOffset) {
        var q = [];
        for (c = 0; c < n.length; c++) {
            if (0 == (v = o[n[c]]).childrenIds.length && !v.isAssistant && "split" != v.templateName)(H = v.parent) && -1 == q.indexOf(H.id) && q.push(H.id)
        }
        for (c = 0; c < q.length; c++) {
            H = o[q[c]], _ = new OrgChart.node(H.id + "_split_0", H.id, [], "split");
            (o[_.id] = _).layout = OrgChart.tree;
            var V = [];
            for (b = H.children.length - 1; 0 <= b; b--) {
                if (OrgChart.server._mixedLayoutAllNodes(H))
                    if (0 == (z = H.children[b]).childrenIds.length && !z.isAssistant && "split" != z.templateName && (z.parent && "split" != z.parent.templateName || z.parent.isAssistant)) {
                        if ((z = H.children[b]).parent = null, z.layout = OrgChart.tree, H.children.splice(b, 1), H.childrenIds.splice(b, 1), a.layout == OrgChart.treeRightOffset && V.splice(0, 0, z.id), a.layout == OrgChart.treeLeftOffset || a.layout == OrgChart.treeRightOffset) {
                            var W;
                            ...
                            OrgChart.templates[I] = {
                                ...
                                nodeMenuButton: "",
                                size: [0, 0]
                            }, (W = new OrgChart.node(z.id + "_transperant", null, [], I)).layout = OrgChart.tree, o[W.id] = W, V.splice(0, 0, W.id)
                        }
            ...
            for (N = 1, b = V.length - 1; 0 <= b; b--) {
                var J;
                if (b % 2 && b != V.length - 1)(J = new OrgChart.node(H.id + "_split_" + N, null, [], "split")).layout = OrgChart.tree, o[J.id] = J, V.splice(b, 0, J.id), N++;
            ...
            for (b = 0; b < V.length; b += 3) {
                ...
                0 != b && (M = o[V[b - 3]]), 0 == b || L || (M = o[V[b - 2]]), E.parent = M, E.pid = M.id, M.children.push(E), H.childrenIds.push(E.id), L && (0 != b && (M = o[V[b - 2]]), L.parent = M, L.pid = M.id, M.children.push(L), H.childrenIds.push(L.id)), B && (0 != b && (M = o[V[b - 1]]), B.parent = M, B.pid = M.id, M.children.push(B), H.childrenIds.push(B.id))
            }
        }
    }
    return {
    ...
    if (t.isGroup && t.groupState == OrgChart.EXPAND) {
        ...
        switch (r) {
            ...
            case OrgChart.orientation.bottom_left:
                ...
                for (var c = 0; c < t.bgnodes.length; c++) h = Math.max(t.bgnodes[c].w, h), d = Math.max(t.bgnodes[c].h, d);
    ...
    if (t.isGroup && t.groupState == OrgChart.EXPAND)
        for (var r = OrgChart.t(t.templateName), i = 0, a = 0, o = 0; o < t.bgnodes.length; o++) {
            var n = t.bgnodes[o];
            n.x = t._groupSettings._cellWidth / 2 - n.w / 2 + r.groupNodesSeparation + (t._groupSettings._cellWidth * i + r.groupNodesSeparation * i), n.y = t._groupSettings._cellHeight / 2 - n.h / 2 + r.groupNodesSeparation + (t._groupSettings._cellHeight * a + r.groupNodesSeparation * a), i >= t._groupSettings._columns - 1 ? (i = 0, a++) : i++
        }
    ...
    var u = {},
        ...
        O = a - n + 2 * i.padding,
        v = o - l + 2 * i.padding,
        y = OrgChart.getScale(s, t, e, i.scaleInitial, i.scaleMax, i.scaleMin, O, v);
    if (C.top = l - i.padding, C.left = n - i.padding, C.bottom = o + i.padding - e / y, C.right = a + i.padding - t / y, C.maxX = a, C.minX = n, C.maxY = o, C.minY = l, null == s && i.align == OrgChart.CENTER) {
        var x = Math.ceil(t / y),
            b = Math.ceil(e / y),
            w = 0,
            _ = 0;
        if (x - 2 * i.padding >= a - n) switch (w = (a + n) / 2 - x / 2, i.orientation) {
            case OrgChart.orientation.right:
            case OrgChart.orientation.right_top:
                w = (n - a) / 2 - x / 2
        } else switch (w = h.x - x / 2 + OrgChart.server._getNodeWidth(h, i) / 2, i.orientation) {
            case OrgChart.orientation.right:
            case OrgChart.orientation.right_top:
        ...
        switch (i.orientation) {
            case OrgChart.orientation.top:
                w = h.x - x / 2 + OrgChart.server._getNodeWidth(h, i) / 2, _ = -i.padding;
                break;
            case OrgChart.orientation.bottom:
                w = h.x - x / 2 + OrgChart.server._getNodeWidth(h, i) / 2, _ = i.padding - b;
                break;
            case OrgChart.orientation.left:
                w = -i.padding, _ = h.y - b / 2 + OrgChart.server._getNodeWidth(h, i) / 2;
                break;
            case OrgChart.orientation.right:
                w = i.padding - x, _ = h.y - b / 2 + OrgChart.server._getNodeWidth(h, i) / 2;
                break;
            case OrgChart.orientation.top_left:
    ...
    node: function(t, e, r, i, a, o, n, l, s, h) {
        ...
        v = v.replace("{opacity}", y), t.isGroup || null == i.nodeMenu || "split" == t.templateName || l === OrgChart.action.exporting || (c += d.nodeMenuButton.replace("{id}", t.id));
        var x = OrgChart.ui._renderGroupedNodes(t, d, i, r, n, a, o, l, s, h);
        return t.isGroup && l != OrgChart.action.exporting && (t.groupState == OrgChart.EXPAND ? c += OrgChart.MINIMIZE.replace("{x}", t.w - 45).replace("{id}", t.id) : c += OrgChart.MAXIMIZE.replace("{x}", t.w - 45).replace("{id}", t.id)), c = v + c + x + OrgChart.grCloseTag
    },
    ...
    if ("multiline" == s) {
        ...
        C.setAttributeNS(null, "x", a), C.setAttributeNS(null, "y", o), C.setAttributeNS(null, "text-anchor", n), C.appendChild(m), d.appendChild(C);
        p = 1;
        for (var O = 1, v = 1; v < f.length; v++) {
            var y = C.firstChild.data.length;
            if (C.firstChild.data += " " + f[v], C.getComputedTextLength() > l) {
                if (C.firstChild.data = C.firstChild.data.slice(0, y), O++, 0 !== c && c < O) break;
                (C = document.createElementNS(h, "tspan")).setAttributeNS(null, "x", a), C.setAttributeNS(null, "y", o + u * p), C.setAttributeNS(null, "text-anchor", n), m = document.createTextNode(f[v]), C.appendChild(m), d.appendChild(C), p++
            }
        }
        var x = "";
        if (null != d.innerHTML) x = d.innerHTML, d.innerHTML = "";
            ...
            var b = "";
            for (v = d.childNodes.length - 1; 0 <= v; v--) b = XMLSerializer().serializeToString(d.childNodes[v]) + b, d.removeChild(d.childNodes[v]);
            x = b
        }
        return x
    }
    ...
    if (1 == i) {
        ...
        if (void 0 !== n.download) {
            ...
            n.setAttribute("href", o), n.setAttribute("download", r), n.style.visibility = "hidden", document.body.appendChild(n), n.click(), document.body.removeChild(n)
        }
    }
    ...
    for (var r, i, a, o, n, l = "", s = 0; s < this.config.clinks.length; s++) {
        var h = this.config.clinks[s],
            ...
            c = t.getBGNode(h.to);
        if ((d.isChildOfGroup || e.res.visibleNodes[d.id]) && ((!d.isChildOfGroup || e.res.visibleNodes[d._groupParentNodeId]) && (c.isChildOfGroup || e.res.visibleNodes[c.id]) && (!c.isChildOfGroup || e.res.visibleNodes[c._groupParentNodeId]))) {
            var g = d.x,
                p = d.y,
                f = c.x,
                u = c.y;
            ...
            switch (this.config.orientation) {
                case OrgChart.orientation.top:
                case OrgChart.orientation.top_left:
                    y <= x ? (_ = 1, O.x = y + d.w / 10, v.x = x - c.w / 10) : (_ = -1, O.x = y - d.w / 10, v.x = x + c.w / 10), p == u ? (_ = 1, O.y = p, v.y = u) : u < p ? (O.y = p, v.y = u + c.h) : (O.y = p + d.h, v.y = u);
                    break;
                case OrgChart.orientation.bottom:
                case OrgChart.orientation.bottom_left:
                    O.x = y <= x ? (_ = -1, y + d.w / 10) : (_ = 1, y - d.w / 10), v.x = x + c.w / 10, p == u ? (_ = -1, O.y = p + d.h, v.y = u + c.h) : u < p ? (O.y = p, v.y = u + c.h) : (O.y = p + d.h, v.y = u);
                    break;
                case OrgChart.orientation.left:
                case OrgChart.orientation.left_top:
    ...
    function t(t, e, r) {
        ...
        (t.parent && t.parent.layout == OrgChart.mixed || t.parent && t.parent.layout == OrgChart.tree) && (l = _.config.mixedHierarchyNodesSeparation);
        var s = {
        ...
        if (t.level == e.level) switch (a = e, _.config.orientation) {
            case OrgChart.orientation.top:
            case OrgChart.orientation.top_left:
                i.push([s.p, s.q]), i.push([s.p, s.q - l / 3]), n = OrgChart.t(a.templateName, S), i.push([a.x + a.w / 2 + n.expandCollapseSize, i[i.length - 1][1]]), i.push([i[i.length - 1][0], a.y]);
                break;
            case OrgChart.orientation.bottom:
            case OrgChart.orientation.bottom_left:
                s.q = t.y + t.h, s.s = I.maxY - l, i.push([s.p, s.q]), i.push([s.r, t.y + t.h + l / 3]), n = OrgChart.t(a.templateName, S), i.push([a.x + a.w / 2 + n.expandCollapseSize, i[i.length - 1][1]]), i.push([i[i.length - 1][0], a.y + a.h]);
                break;
            case OrgChart.orientation.left:
            case OrgChart.orientation.left_top:
                s.p = t.x, s.q = t.y + t.h / 2 + n.expandCollapseSize, s.r = I.minX - l, s.s = t.y + t.h / 2 + n.expandCollapseSize, i.push([s.p, s.q]), i.push([t.x - l / 3, s.q]), n = OrgChart.t(a.templateName, S), i.push([i[i.length - 1][0], a.y + a.h / 2 + n.expandCollapseSize]), i.push([a.x, i[i.length - 1][1]]);
                break;
            case OrgChart.orientation.right:
            case OrgChart.orientation.right_top:
                s.p = t.x + t.w, s.q = t.y + t.h / 2 + n.expandCollapseSize, s.r = I.maxX + l, s.s = t.y + t.h / 2 + n.expandCollapseSize, i.push([s.p, s.q]), i.push([t.x + t.w + l / 3, s.q]), n = OrgChart.t(a.templateName, S), i.push([i[i.length - 1][0], a.y + a.h / 2 + n.expandCollapseSize]), i.push([a.x + a.w, i[i.length - 1][1]])
        } else {
            switch (_.config.orientation) {
                case OrgChart.orientation.top:
                case OrgChart.orientation.top_left:
                ...
                case OrgChart.orientation.bottom:
                case OrgChart.orientation.bottom_left:
                    s.q = t.y + t.h, s.s = I.maxY - l, i.push([s.p, s.q]), i.push([s.r, t.y + t.h + l / 3]);
                    break;
                case OrgChart.orientation.left:
                case OrgChart.orientation.left_top:
                    s.p = t.x, s.q = t.y + t.h / 2 + n.expandCollapseSize, s.r = I.minX - l, s.s = t.y + t.h / 2 + n.expandCollapseSize, i.push([s.p, s.q]), i.push([t.x - l / 3, s.q]);
                    break;
                case OrgChart.orientation.right:
                case OrgChart.orientation.right_top:
                    s.p = t.x + t.w, s.q = t.y + t.h / 2 + n.expandCollapseSize, s.r = I.maxX + l, s.s = t.y + t.h / 2 + n.expandCollapseSize, i.push([s.p, s.q]), i.push([t.x + t.w + l / 3, s.q])
            }
            for (var h = t; null == a;) {
                var d = !1,
                    c = h.parent,
                    g = c.leftNeighbor,
                    p = c.rightNeighbor;
                if (c.id == e.id ? a = c : OrgChart._intersects(c, s, _.config) && (s = OrgChart._addPoint(c, i, _.config, s, o), d = !0), c.id != e.id) {
                    for (; g;) {
                        if (g.id == e.id) {
                            a = g;
                            break
                        }
                        OrgChart._intersects(g, s, _.config) && (s = OrgChart._addPoint(g, i, _.config, s, o), d = !0), g = g.leftNeighbor
                    }
                    for (; p;) {
                        if (p.id == e.id) {
                            a = p;
                        ...
                        OrgChart._intersects(p, s, _.config) && (s = OrgChart._addPoint(p, i, _.config, s, o), d = !0), p = p.rightNeighbor
                    }
                }
                if (!d) {
                    var f = i[i.length - 1][0],
                        u = 0;
                    if (c.parent) switch (l = _.config.levelSeparation, c.parent.layout != OrgChart.mixed && c.parent.layout != OrgChart.tree || (l = _.config.mixedHierarchyNodesSeparation), _.config.orientation) {
                        case OrgChart.orientation.top:
                        case OrgChart.orientation.top_left:
                        ...
                        case OrgChart.orientation.left:
                        case OrgChart.orientation.left_top:
                            f = c.parent.x + c.parent.w + l * (2 / 3), u = i[i.length - 1][1];
                            break;
                        case OrgChart.orientation.right:
                        case OrgChart.orientation.right_top:
            ...
            switch (n = OrgChart.t(a.templateName, S), i.splice(i.length - 1, 1), _.config.orientation) {
                case OrgChart.orientation.top:
                case OrgChart.orientation.top_left:
                    i.push([a.x + a.w / 2 + n.expandCollapseSize, i[i.length - 1][1]]), i.push([i[i.length - 1][0], a.y + a.h]);
                    break;
                case OrgChart.orientation.bottom:
                case OrgChart.orientation.bottom_left:
                    i.push([a.x + a.w / 2 + n.expandCollapseSize, i[i.length - 1][1]]), i.push([i[i.length - 1][0], a.y]);
                    break;
                case OrgChart.orientation.left:
                case OrgChart.orientation.left_top:
                    i.push([i[i.length - 1][0], a.y + a.h / 2 + n.expandCollapseSize]), i.push([a.x + a.w, i[i.length - 1][1]]);
                    break;
                case OrgChart.orientation.right:
                case OrgChart.orientation.right_top:
                    i.push([i[i.length - 1][0], a.y + a.h / 2 + n.expandCollapseSize]), i.push([a.x, i[i.length - 1][1]])
            }
        }
        var C = N.template;
        C || (C = "orange");
        ...
        var y = i.join(" ");
        if (A += '<g c-link-from="{from}" c-link-to="{to}">'.replace("{from}", t.id).replace("{to}", e.id) + n.link.replace("{d}", y), N.label) {
            var x = n.label.replace("{x}", m.x).replace("{y}", m.y).replace("{val}", N.label),
            ...
            A += n.label.replace("{x}", m.x).replace("{y}", m.y + w).replace("{val}", N.label)
        }
